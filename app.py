...
THRESHOLD_MAX_ACC_FALL_SCORE = 19.0  # <--- ADJUSTED: From 20.0 to 19.0
THRESHOLD_MIN_ACC_FALL_SCORE = 2.0   # Unchanged
THRESHOLD_ACC_DIFF_FALL_SCORE = 11.0 # <--- ADJUSTED: From 12.0 to 11.0
THRESHOLD_MAX_GYRO_FALL_SCORE = 4.5  # <--- ADJUSTED: From 5.0 to 4.5

THRESHOLD_IMPACT_PRE_ACC = 5.0      # Unchanged
THRESHOLD_IMPACT_POST_ACC = 12.0    # Unchanged

# Hybrid decision logic thresholds (from predict endpoint in notebook's test function)
OVERRIDE_FALL_SCORE_HIGH = 0.95    # Unchanged
OVERRIDE_MAX_ACC_EXTREME = 40.0    # Unchanged
OVERRIDE_MIN_ACC_EXTREME = 0.5     # Unchanged

ML_CONF_FALL_HIGH = 0.7            # Unchanged
ML_CONF_FALL_MEDIUM = 0.6          # Unchanged
ML_CONF_FALL_LOW = 0.52            # Unchanged

# ADJUSTED: Stricter sudden change for fall classification
THRESHOLD_MAX_CHANGE_AMBIGUOUS = 15.0 # Unchanged
THRESHOLD_MIN_ACC_AMBIGUOUS = 2.0    # Unchanged
THRESHOLD_IMPACT_AMBIGUOUS = 12.0    # Unchanged

# NEW: Threshold for combining sudden change with rotation
THRESHOLD_MAX_CHANGE_ROTATION_COMBINED = 8.0 # Unchanged

# Post-prediction confirmation (New concept for this problem)
CONFIRMATION_WINDOWS = 3 # Unchanged
STILLNESS_ACC_STD_THRESHOLD = 0.5 # Unchanged
STILLNESS_GYRO_MAX_THRESHOLD = 0.5 # Unchanged
...
THRESHOLD_STILL_CONFIDENCE = 0.99 # Unchanged
...
def predict(data: FallDetectionData):
    ...
    try:
        ...
        # --- High-priority check for stillness ---
        ...
        if is_still(raw_df_window):
            ...
            prediction_probs = {
                ...
                for i in range(len(classes) - 1 if 'steady_not_moving' in classes and len(prediction[0]) == 3 else len(classes)) 
                # This ensures we only extract for the 3 original ML classes if 'steady_not_moving' is a new,
                # rule-based class and not part of the ML model's output layer.
                # If ML was trained with 4 classes, then use len(classes).
            }
...
def predict_raw(data: FallDetectionRawData):
    ...
    try:
        ...
        predicted_class_index = prediction.argmax(axis=1)[0]
        # Adjust class indexing for predict_raw if 'steady_not_moving' is not part of ML model's output
        if 'steady_not_moving' in classes and len(prediction[0]) == 3: # Assuming ML only outputs 3 classes
            # Map index 0->falling, 1->kneeling, 2->walking
            predicted_class = ['falling', 'kneeling', 'walking'][predicted_class_index]
        else: # If ML model was trained with 'steady_not_moving' as a class, or other mapping
